...
from corporate_earnings_engine.providers.base import EarningsProvider, EarningsObservation
...
class ProviderRouter:
    ...
    SYMBOL_EXCHANGE = {
        ...
        "BP.L": "LSE", "SHEL.L": "LSE", "HSBA.L": "LSE",
        "9984.T": "TSE", "6758.T": "TSE", "8306.T": "TSE",
        "NESN.SW": "SIX", "ROG.SW": "SIX", "UBSG.SW": "SIX",
        "RY.TO": "TSX", "TD.TO": "TSX", "BNS.TO": "TSX",
        "BHP.AX": "ASX", "CBA.AX": "ASX", "CSL.AX": "ASX",
        "AIA.NZ": "NZX", "FPH.NZ": "NZX",
    }
    ...
    def get_providers_for_symbol(self, symbol: str) -> List[EarningsProvider]:
        ...
        provider_names = self.EXCHANGE_PROVIDERS.get(exchange, self.EXCHANGE_PROVIDERS["NASDAQ"])
        providers = []
    ...
    def get_earnings_with_fallback(
        ...
        for provider in providers:
            ...
            try:
                ...
                if data:
                    print(f"[Router] ✅ {symbol} → {provider.get_provider_name()} → {len(data)} records")
                    self._cache[cache_key] = data
                    return data
                else:
                    print(f"[Router] ⚠️ {symbol} → {provider.get_provider_name()} → No data")
            except Exception as e:
                print(f"[Router] ❌ {symbol} → {provider.get_provider_name()} → {str(e)[:50]}")
                continue
    ...
    def get_earnings_for_symbols(
        ...
        for symbol in symbols:
            results[symbol] = self.get_earnings_with_fallback(symbol, start_date, end_date)
        return results
