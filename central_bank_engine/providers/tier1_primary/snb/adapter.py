...
class SNBAdapter:
    def adapt(self, raw: Dict[str, Any], provider_name: str) -> UniversalCentralBankEvent:
        ...
        return UniversalCentralBankEvent(
            event_id=raw.get("event_id", f"snb_" + release_time.isoformat()),
            provider=provider_name,
            bank=raw.get("bank", "Swiss National Bank"),
            country=raw.get("country", "CH"),
            currency=raw.get("currency", "CHF"),
            event_type=raw.get("event_type", "RateDecision"),
            title=raw.get("title", "Swiss National Bank Rate Decision"),
            ...
            governor=raw.get("governor", "Thomas Jordan"),
