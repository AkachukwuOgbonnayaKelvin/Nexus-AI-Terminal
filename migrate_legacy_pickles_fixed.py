...
def decode_timestamp_bytes(data: bytes) -> datetime:
    ...
    year (2 bytes, big-endian), month, day, hour, minute, second, microsecond (4 bytes)
    """
    if len(data) < 8:
...
def extract_timestamp(obj: Any) -> datetime:
    """
    Recursively extract a datetime from a legacy object.
    Handles bytes, DummyOHLCVData, and nested structures.
    """
    if obj is None:
    ...
    # If it's a dict, look for timestamp keys
    if isinstance(obj, dict):
        for key in ["timestamp", "time", "date", "datetime"]:
            if key in obj:
                return extract_timestamp(obj[key])
    # If it's a list, try first element (should not happen, but robust)
    if isinstance(obj, list) and obj:
        return extract_timestamp(obj[0])
    # If it's any object with __dict__, try common attributes
    if hasattr(obj, "__dict__"):
        for attr in ["timestamp", "time", "date", "datetime"]:
            if hasattr(obj, attr):
                return extract_timestamp(getattr(obj, attr))
    # Fallback: if it's a number, treat as timestamp
...
def extract_candle(obj: Any) -> dict:
    """Convert a single legacy candle object to a dict."""
    # Extract timestamp
