...
class MKT001Auditor(BaseAuditor):
    ...
    def audit_provider(self) -> AuditResult:
        ...
        try:
            ...
            if not provider.connect():
                return AuditResult(
                    name="Provider", status="FAIL", message="MT5 connection failed"
                )
            ...
            if len(found) > 0:
                return AuditResult(
                    ...
                    details={"found": found, "total_symbols": len(symbols)},
                )
            else:
                return AuditResult(
                    name="Provider",
    ...
    def audit_ndip(self) -> AuditResult:
        ...
        try:
            ...
            return AuditResult(
                name="NDIP", status="WARNING", message=f"NDIP publisher not found: {e}"
            )
    ...
    def audit_parity(self) -> AuditResult:
        try:
            ...
            if total_bars > 0:
                ...
                return AuditResult(
                    name="Parity", status="FAIL", message="No data in warehouse"
                )

        except Exception as e:
            return AuditResult(
                name="Parity", status="FAIL", message=f"Parity check error: {e}"
            )
