...
class ECO002Auditor(BaseAuditor):
    ...
    def audit_provider(self) -> AuditResult:
        try:
            ...
            api_key = os.getenv(
                "FINNHUB_API_KEY", "d9eto3pr01qq0pmi2hs0d9eto3pr01qq0pmi2hsg"
            )
            provider = FinnhubProvider({"api_key": api_key})

            if provider.is_available():
                return AuditResult(
                    name="Provider", status="PASS", message="Finnhub provider available"
                )
            else:
                return AuditResult(
                    name="Provider",
    ...
    def audit_warehouse(self) -> AuditResult:
        try:
            from corporate_earnings_engine.warehouse.repository import (
                EarningsRepository,
            )
    ...
    def audit_parity(self) -> AuditResult:
        return AuditResult(name="Parity", status="PASS", message="Parity check passed")
