...
class EIVGate:
    ...
    def run(self, engine_id: str) -> Dict[str, Any]:
        ...
        result["score"] = int((passed / total_weighted) * 100) if total_weighted > 0 else 0
            ...
            if "integration" in data:
                ...
                if isinstance(integration, dict):
                    for key in integration:
                        result["diagnostics"]["found_fields"].append(f"integration.{key}")
            
            # Determine status
            if result["diagnostics"]["missing_fields"]:
                result["status"] = "FAIL"
                result["message"] = f"Missing required fields: {', '.join(result['diagnostics']['missing_fields'])}"
                result["fix"] = f"Add missing fields to engine.yaml: {', '.join(result['diagnostics']['missing_fields'])}"
    ...
    def _check_provider_connection(self, engine_path: Path) -> Dict[str, Any]:
        ...
        for path in provider_paths:
            if path.exists() and any(path.glob("*.py")):
                return {"status": "PASS", "message": f"Provider modules found in {path.name}"}
    ...
    def _check_ndip_connection(self, engine_path: Path) -> Dict[str, Any]:
        ...
        return {"status": "WARN", "message": "No NDIP publication modules found (optional)"}
    ...
    def _check_dar_registration(self, engine_path: Path) -> Dict[str, Any]:
        ...
        if engine_yaml.exists():
            try:
                ...
                pass
        return {"status": "WARN", "message": "DAR integration not configured (optional)"}
