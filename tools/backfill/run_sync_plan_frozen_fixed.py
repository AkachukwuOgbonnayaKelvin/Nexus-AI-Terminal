...
def run_sync_plan_frozen(symbol: str, days_back: int = 3):
    ...
    for idx, gap in enumerate(plan.missing_ranges):
        ...
        print(f"\n=== Running job_{idx:04d}: {desc} ===")
        print(f"  Gap: {gap.start} -> {gap.end}")

        # Always start from the gap start
        current = gap.start
        end = gap.end

        # Check if checkpoint is inside this gap and valid
        last_completed = checkpoint.get("last_completed")
        if last_completed:
            last_completed = datetime.fromisoformat(last_completed)
            # Only resume if the checkpoint is within this gap
            if last_completed > current and last_completed < end:
                print(f"  Resuming from checkpoint inside gap: {last_completed}")
                # But we still start from current? No, we should start from the checkpoint
                # Actually, to be safe, we'll still start from gap.start to ensure we don't miss ticks.
                # However, we can optionally skip to the checkpoint, but we must verify coverage.
                # The safest: start from gap.start. Duplicates will be skipped.
            else:
                print(f"  Checkpoint is outside this gap ({last_completed}), ignoring.")
        else:
            print("  No checkpoint found, starting from gap start.")
        ...
        print(f"  Job complete. Total inserted: {total_inserted}")

    # Process incremental gap if it exists and wasn't already included in missing_ranges
    if plan.incremental_range:
        print("\n=== Running job_incremental: incremental sync ===")
        gap = plan.incremental_range
        current = gap.start
        end = gap.end
        print(f"  Gap: {current} -> {end}")
        ...
        print(f"  Incremental complete. Total inserted: {total_inserted}")

    # Verify against frozen target
