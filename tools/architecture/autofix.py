...
class AutoFixer:
    ...
    def _fix_engine_imports(self) -> None:
        ...
        for py_file in (self.root / "tools" / "architecture").rglob("*.py"):
            if any(e in str(py_file) for e in ["__pycache__", ".venv", "bootstrap_check.py", "autofix.py"]):
                continue
            try:
                ...
                if "from tools.architecture.engine import" in content:
                    ...
                    if new_content != content:
                        self.fixes.append((py_file, "import", "from engine.py → models.py"))
                if "from ..engine import" in content:
                    ...
                    if new_content != content:
                        self.fixes.append((py_file, "import", "from ..engine → models.py"))
                if new_content != content:
    ...
    def _fix_models_imports(self) -> None:
        ...
        for py_file in (self.root / "tools" / "architecture" / "validators").glob("*.py"):
            if py_file.name in ["__init__.py", "base.py", "registry.py"]:
                continue
            try:
                with open(py_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                if "ARCResult" in content and "from tools.architecture.models import ARCResult" not in content:
                    if "from .base import" not in content:
                        new_content = "from tools.architecture.models import ARCResult\n" + content
                        self.fixes.append((py_file, "import", "Added ARCResult import from models.py"))
                        py_file.write_text(new_content, encoding='utf-8')
            except Exception:
                pass
            ...
            try:
                ...
                if "class " in content and "Validator" in py_file.name:
                    if "BaseValidator" not in content:
                        ...
                        new_lines = []
                        for line in lines:
                            if line.strip().startswith("class ") and "Validator" in line and "(" not in line:
                                line = line.replace("class ", "class ") + "(BaseValidator)"
                            new_lines.append(line)
                        if not any("from tools.architecture.validators.base import BaseValidator" in l for l in new_lines):
                            new_lines.insert(0, "from tools.architecture.validators.base import BaseValidator")
                        new_content = '\n'.join(new_lines)
                        if new_content != content:
                            self.fixes.append((py_file, "inheritance", "Added BaseValidator inheritance"))
                            py_file.write_text(new_content, encoding='utf-8')
            except Exception:
                pass
    ...
    def _fix_registry_imports(self) -> None:
        ...
        registry_file = self.root / "tools" / "architecture" / "validators" / "registry.py"
        ...
        try:
            ...
            if "from tools.architecture.engine import" in content:
                ...
                self.fixes.append((registry_file, "import", "Fixed registry engine import"))
