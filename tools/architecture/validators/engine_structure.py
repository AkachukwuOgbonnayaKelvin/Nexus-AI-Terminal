...
class EngineStructureValidator(BaseValidator):
    ...
    REQUIRED_FOLDERS = [
        "acquisition", "providers", "warehouse", "publication",
        "runtime", "configuration", "tests", "gateway"
    ]
    ...
    def validate(self) -> List[ARCResult]:
        ...
        if not engines:
            results.append(self.result(False, "No engines found in project", severity="critical"))
            return results

        for engine_path in engines:
            engine_name = engine_path.name
            missing = [f for f in self.REQUIRED_FOLDERS if not (engine_path / f).exists()]
            ...
            if missing:
                ...
                results.append(self.result(True, f"{engine_name} has complete structure"))
