...
class RuntimeValidator(BaseValidator):
    ...
    def validate(self) -> List[ARCResult]:
        ...
        if not runtime_file.exists():
            results.append(self.result(False, "DAR-001 runtime.py not found", severity="critical"))
            return results

        registry_file = self.root / "runtime" / "engine_registry.py"
        if not registry_file.exists():
            results.append(self.result(False, "Engine registry not found", severity="high"))
        else:
