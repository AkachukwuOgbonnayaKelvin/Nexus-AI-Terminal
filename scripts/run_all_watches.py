...
for sym in symbols:
    try:
        ...
        print(f"\n===== {sym} =====")
        print(f"Macro: {watch.macro_bias} | Context: {watch.context_bias} | Exec: {watch.execution_bias}")
        ...
        if watch.distance_to_zone is not None:
            print(f"Distance to Zone: {watch.distance_to_zone:.5f} ({watch.distance_atr:.2f} ATR)")
        if watch.time_to_zone_min is not None:
            print(f"Time to Zone: {watch.time_to_zone_min:.1f}–{watch.time_to_zone_max:.1f} hours")
        ...
        for cat, conds in watch.conditions.items():
            ...
            if not_met:
                print(f"  {cat.upper()}: ❌ " + ", ".join([c['label'] for c in not_met]))
