...
for sym in symbols:
    print(f"\n===== {sym} =====")
    result = engine.analyze_mtf(sym, timeframes=['D1', 'H4', 'H1', 'M15'], lookback_bars=200)
    ...
    for tf, data in result['timeframes'].items():
        print(f"  {tf}: bias={data['bias']}, conf={data['confidence']:.2f}, regime={data['regime']}, phase={data['phase']}")
