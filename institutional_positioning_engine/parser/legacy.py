        ...
        for row in data:
            try:
                ...
                record = {
                    ...
                    "commercial_long": self._safe_int(
                        self._extract_field(row, "Commercial Long", "commercial_long")
                    ),
                    ...
                    "commercial_spread": self._safe_int(
                        self._extract_field(
                            row, "Commercial Spread", "commercial_spread"
                        )
                    ),
                    # Non-commercial positions
                    "non_commercial_long": self._safe_int(
                        self._extract_field(
                            row, "Non-Commercial Long", "non_commercial_long"
                        )
                    ),
                    "non_commercial_short": self._safe_int(
                        self._extract_field(
                            row, "Non-Commercial Short", "non_commercial_short"
                        )
                    ),
                    "non_commercial_spread": self._safe_int(
                        self._extract_field(
                            row, "Non-Commercial Spread", "non_commercial_spread"
                        )
                    ),
                    # Nonreportables
