        ...
        for row in data:
            try:
                ...
                record = {
                    ...
                    # Dealer/Intermediary
                    "dealer_long": self._safe_int(self._extract_field(row, "Dealer Long", "dealer_long")),
                    "dealer_short": self._safe_int(self._extract_field(row, "Dealer Short", "dealer_short")),
                    "dealer_spread": self._safe_int(self._extract_field(row, "Dealer Spread", "dealer_spread")),
                    # Asset Manager/Institutional
                    "asset_manager_long": self._safe_int(
                        self._extract_field(row, "Asset Manager Long", "asset_manager_long")
                    ),
                    "asset_manager_short": self._safe_int(
                        self._extract_field(row, "Asset Manager Short", "asset_manager_short")
                    ),
                    "asset_manager_spread": self._safe_int(
                        self._extract_field(row, "Asset Manager Spread", "asset_manager_spread")
                    ),
                    # Leveraged Funds
                    "leveraged_long": self._safe_int(
                        self._extract_field(row, "Leveraged Funds Long", "leveraged_long")
                    ),
                    "leveraged_short": self._safe_int(
                        self._extract_field(row, "Leveraged Funds Short", "leveraged_short")
                    ),
                    "leveraged_spread": self._safe_int(
                        self._extract_field(row, "Leveraged Funds Spread", "leveraged_spread")
                    ),
                    # Other Reportables
