...
class COTGateway:
    def __init__(self, warehouse: Optional[COTWarehouse] = None):
        self.warehouse = warehouse or COTWarehouse()

    async def get_latest_report(self, market_code: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def get_market_history(self, market_code: str, limit: int = 100) -> List[Dict[str, Any]]:
        """Get historical positions for a market."""
        return await self.warehouse.get_market_history(market_code, limit)

    async def get_reports_between(self, start_date: str, end_date: str) -> List[Dict[str, Any]]:
