...
class EconomicEventsSynchronizer:
    def __init__(self):
        self.core_conn = None
    ...
    def get_last_sync_timestamp(self):
        with self.asset_conn.cursor() as cur:
            cur.execute("""
                SELECT last_timestamp FROM raw.sync_state WHERE stream_name = 'economic_events'
            """)
            result = cur.fetchone()
    ...
    def update_sync_state(self, last_timestamp):
        with self.asset_conn.cursor() as cur:
            cur.execute("""
                ...
                WHERE stream_name = 'economic_events'
            """, (last_timestamp,))
    ...
    def upsert_events(self, events):
        ...
        conflict_cols = ['event_id', 'release_time_utc']
        update_cols = [col for col in columns if col not in conflict_cols + ['created_at']]
        update_set = ', '.join([f'{col} = EXCLUDED.{col}' for col in update_cols])

        query = f"""
            INSERT INTO raw.economic_events ({', '.join(columns)})
            VALUES ({placeholders})
    ...
    def sync_once(self):
        try:
            ...
            logger.info("Syncing economic events since: %s", last_ts)
            ...
            latest_ts = events[-1]['release_time_utc']
            self.update_sync_state(latest_ts)

            logger.info("Synced %d events (up to %s)", count, latest_ts)
            return count
...
if __name__ == "__main__":
    ...
    print(f"Synced {count} economic events.")
