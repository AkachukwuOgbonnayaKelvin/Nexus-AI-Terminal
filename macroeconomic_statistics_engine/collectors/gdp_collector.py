...
class GDPCollector(BaseCollector):
    ...
    def collect(self) -> list[MacroObservation]:
        ...
        for country in self.get_countries():
            ...
            providers = self.registry.get_providers_for_indicator(
                self._indicator, country
            )

            for provider in providers:
                if provider.is_available():
                    print(
                        f"  Using {provider.get_provider_name()} (Tier {provider.get_tier()})"
                    )
    ...
    def get_historical_coverage(self) -> dict[str, Any]:
        ...
        for country in self.get_countries():
            ...
            if country_obs:
                ...
                coverage[country] = {"count": 0, "status": "No data"}
