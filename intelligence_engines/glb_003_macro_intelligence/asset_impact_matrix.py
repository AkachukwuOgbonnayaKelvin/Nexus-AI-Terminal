...
from intelligence.schemas.asset_impact import AssetImpact, AssetImpactMatrix, AssetType, Direction, ImpactStatus, ImpactDriver
...
class MacroAssetImpactMatrix:
    ...
    COMMODITIES = ["XAUUSD", "XAGUSD", "XPTUSD", "XPDUSD", "WTI", "BRENT", "NGAS", "COPPER"]
    EQUITIES = ["US500", "US100", "US30", "GER40", "UK100", "FRA40", "JP225", "HK50", "AU200", "CN50", "IN50", "BR60"]
    BONDS = ["US2Y", "US10Y", "US30Y", "BUND", "GILT", "JGB"]
    CRYPTO = ["BTCUSD", "ETHUSD", "SOLUSD", "XRPUSD"]
    
    @classmethod
    def generate(cls, macro_score: float, growth_score: float, inflation_score: float, 
                 employment_score: float, confidence: float) -> AssetImpactMatrix:
        ...
        all_assets = cls.FX_PAIRS + cls.COMMODITIES + cls.EQUITIES + cls.BONDS + cls.CRYPTO
    ...
    @classmethod
    def _generate_impact(cls, asset: str, sentiment: float, confidence: float) -> AssetImpact:
        ...
        if asset in cls.FX_PAIRS:
            ...
            return AssetImpact(
                ...
                drivers=[ImpactDriver(name=d, direction=cls._get_direction(score), strength=0.7) for d in drivers[:2]],
                relevance=0.80,
                engine_id="GLB-003",
                engine_name="Macro Intelligence Engine"
            )
        ...
        if asset in cls.COMMODITIES:
            ...
            return AssetImpact(
                ...
                status=ImpactStatus.ANALYZED,
                drivers=[ImpactDriver(name=d, direction=cls._get_direction(score), strength=0.6) for d in drivers[:2]],
                relevance=0.60,
                engine_id="GLB-003",
                engine_name="Macro Intelligence Engine"
            )
        ...
        if asset in cls.EQUITIES:
            if asset in ["US100", "US500", "US30"]:
                multiplier = 1.0 if asset == "US100" else 0.9 if asset == "US500" else 0.8
                score = sentiment * multiplier
                drivers = ["Economic growth", "Earnings"] if asset in ["US100", "US500"] else ["Growth"]
            else:
                score = sentiment * 0.7
        ...
        if asset in cls.BONDS:
            ...
            return AssetImpact(
                ...
                status=ImpactStatus.ANALYZED,
                drivers=[ImpactDriver(name="Growth expectations", direction=cls._get_direction(score), strength=0.5)],
                relevance=0.50,
                engine_id="GLB-003",
                engine_name="Macro Intelligence Engine"
            )
        ...
        if asset in cls.CRYPTO:
            ...
            return AssetImpact(
                ...
                status=ImpactStatus.ANALYZED,
                drivers=[ImpactDriver(name="Risk appetite", direction=cls._get_direction(score), strength=0.6)],
                relevance=0.45,
                engine_id="GLB-003",
                engine_name="Macro Intelligence Engine"
            )
