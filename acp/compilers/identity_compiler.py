...
class IdentityCompiler:
    ...
    def compile(self, engine, verbose: bool = False, fix: bool = False) -> Dict[str, Any]:
        ...
        total_checks += 1
        if engine.id and engine.id.startswith(tuple(["GLO", "AST", "TEC", "INS", "IMT", "SEN", "MEM", "NEWS"])):
        ...
        if engine.purpose and len(engine.purpose) > 10:
            checks["purpose"] = {"status": "Pass", "value": engine.purpose[:50] + "..." if len(engine.purpose) > 50 else engine.purpose}
            passed_checks += 1
        else:
            checks["purpose"] = {
                "status": "Fail",
        ...
        valid_classifications = [
            "Data Acquisition", "Data Processing", "Domain Intelligence",
            "Analytics", "Execution", "Orchestration", "Gateway"
        ]
        if engine.classification and engine.classification in valid_classifications:
            checks["classification"] = {"status": "Pass", "value": engine.classification}
            passed_checks += 1
        else:
            ...
            result["fix"] = f"Set classification: One of {', '.join(valid_classifications)}"
