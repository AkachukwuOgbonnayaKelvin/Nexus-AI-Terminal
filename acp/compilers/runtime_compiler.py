...
class RuntimeCompiler:
    ...
    def compile(self, engine, verbose: bool = False, fix: bool = False) -> Dict[str, Any]:
        ...
        if lifecycle:
            ...
            result["impact"] = "DAR cannot start/stop/recover/schedule/monitor this engine"
        ...
        if files_check["missing"]:
            ...
            if result["status"] != "Critical":
                result["status"] = "Critical"
            result["issue"] = f"Missing runtime files: {', '.join(files_check['missing'])}"
            result["impact"] = "DAR cannot run this engine"
            result["fix"] = f"Create missing files in runtime/"
        else:
            checks["runtime_files"] = {"status": "Pass", "present": files_check["present"]}
