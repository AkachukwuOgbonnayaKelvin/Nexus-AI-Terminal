...
from acp.output.visualizer import Visualizer
...
def _run_scan(project_root: Path, verbose: bool = False):
    ...
    for engine_dict in result['engines']:
        ...
        if dep_result['dependencies']:
            print(f"        Internal Dependencies: {', '.join(dep_result.get('internal_dependencies', []))}")
            print(f"        External Dependencies: {', '.join(dep_result.get('external_dependencies', []))}")
...
def _run_engine_analysis(project_root: Path, engine_id: str, verbose: bool = False):
    ...
    print("")
    print("Components:")
    print(f"  Required Folders Present: {len(arch_result['present_folders'])}/{len(arch_result['present_folders']) + len(arch_result['missing_folders'])}")
    if arch_result['missing_folders']:
        print(f"    Missing: {', '.join(arch_result['missing_folders'])}")
    
    print(f"  Required Files Present: {len(arch_result['present_files'])}/{len(arch_result['present_files']) + len(arch_result['missing_files'])}")
    if arch_result['missing_files']:
        print(f"    Missing: {', '.join(arch_result['missing_files'])}")
    
    if arch_result['optional_present']:
        print(f"  Optional Components Present: {', '.join(arch_result['optional_present'])}")
    
    if arch_result['optional_missing']:
        print(f"  Optional Components Missing: {', '.join(arch_result['optional_missing'])}")
...
def _run_repair_plan(project_root: Path):
    ...
    if repair_items:
        ...
        for i, item in enumerate(repair_items, 1):
            priority_icon = "[CRITICAL]" if item["priority"] == "Critical" else "[HIGH]" if item["priority"] == "High" else "[INFO]"
            print("")
            print(f"{i}. {priority_icon} [{item['priority']}] {item['engine']} - {item['type']}")
