...
class ArchitectureOS:
    ...
    def compile(self, engine_id: Optional[str] = None, 
        ...
        if engine_id:
            ...
            if not engines:
                return {
                    ...
                    "build_decision": {"blocked": True, "reason": f"Engine {engine_id} not found"}
                }
        ...
        for engine in engines:
            ...
            for compiler_name, compiler in self.compilers.items():
                ...
                if result.get("status") == "Critical":
                    ...
                    results["build_decision"]["reason"] = f"Critical failures in {engine.id}"
            
            # Calculate score
            scores = [r.get("score", 0) for r in engine_result["layers"].values()]
            engine_result["overall_score"] = int(sum(scores) / len(scores)) if scores else 0
        ...
        for layer, values in scores.items():
            results["platform_score"][layer] = int(sum(values) / len(values)) if values else 0
        results["platform_score"]["overall"] = int(sum(results["platform_score"].values()) / len(results["platform_score"]))
                ...
                for layer, layer_result in engine.get("layers", {}).items():
                    if layer_result.get("status") in ["Critical", "Warning"]:
                        results["repair_plan"].append({
                            "priority": "Critical" if layer_result["status"] == "Critical" else "Warning",
                            "engine": engine["id"],
                            "layer": layer,
                            "issue": layer_result.get("issue", "Unknown"),
                            "fix": layer_result.get("fix", "Unknown"),
                            "estimated_effort": layer_result.get("estimated_effort", "Unknown")
                        })
