            ...
            if dep_path.exists():
                ...
                for pattern in ["*_engine", "engine_*", "*-engine"]:
                    for found_path in self.project_root.glob(pattern):
                        if found_path.name.lower().replace("_engine", "").replace("-engine", "") == dep.lower():
                            result["internal_dependencies"].append(dep)
                            break
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        ...
                        if module not in result["dependencies"] and not module.startswith('_'):
                ...
                if dep_engine_yaml.exists():
                    try:
                        ...
                        if engine_id.lower() in [d.lower() for d in dep_deps]:
                            result["circular_dependencies"].append(f"{engine_id} -> {dep} -> {engine_id}")
                    except:
                        pass
