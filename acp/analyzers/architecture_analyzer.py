...
class ArchitectureAnalyzer:
    ...
    def __init__(self):
        self.required_folders = ["acquisition", "warehouse", "publication"]

        self.optional_folders = ["runtime", "gateway", "observability", "tests", "docs"]

        self.required_files = ["engine.yaml", "contract.yaml"]
    ...
    def analyze(self, engine_path: Path, engine_data: dict) -> dict[str, Any]:
        ...
        result["compliance_score"] = (
            int((required_actual / required_total) * 100) if required_total > 0 else 100
        )
                ...
                result["info"].append(
                    {
                        ...
                        "recommendation": "Add only if required by engine specification",
                    }
                )
        ...
        result["maturity_score"] = (
            int((optional_actual / optional_total) * 100) if optional_total > 0 else 0
        )

        # Calculate overall score (weighted: 70% compliance, 30% maturity)
        result["overall_score"] = int(
            (result["compliance_score"] * 0.7) + (result["maturity_score"] * 0.3)
        )
