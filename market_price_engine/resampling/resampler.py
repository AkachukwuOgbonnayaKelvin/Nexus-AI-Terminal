...
class TimeframeResampler:
    ...
    def resample(self, bars: List[OHLCVData], target_timeframe: str) -> List[OHLCVData]:
        ...
        for bar in sorted_bars:
            ...
            if current_group_start is None:
                current_group_start = self._get_group_start(bar.timestamp, target_minutes)
            ...
            if bar_group_start == current_group_start:
                ...
                if current_group:
                    resampled = self._resample_group(current_group, current_group_start, target_timeframe)
                    if resampled:
    ...
    def _resample_group(self, group: List[OHLCVData], timestamp: datetime, timeframe: str) -> Optional[OHLCVData]:
        ...
        total_volume = sum([b.volume for b in group if b.volume is not None]) if group[0].volume is not None else None
    ...
    def resample_all_timeframes(self, bars: List[OHLCVData]) -> Dict[str, List[OHLCVData]]:
