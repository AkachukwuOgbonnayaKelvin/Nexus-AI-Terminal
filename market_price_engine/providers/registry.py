...
class ProviderRegistry:
    ...
    def _initialize_providers(self):
        ...
        print(f"[REGISTRY] MT5 PRIMARY - Terminal: {mt5_config.get('terminal_path', 'default')}")
    ...
    def get_priority_order(self) -> List[str]:
        ...
        return sorted(
            self.providers.keys(),
            key=lambda x: priority_map.get(self.priorities.get(x, ProviderPriority.BACKUP), 4)
        )
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        for name, provider in self.providers.items():
            ...
            health["priority"] = self.priorities.get(name, ProviderPriority.BACKUP).value
            status[name] = health
        return status
