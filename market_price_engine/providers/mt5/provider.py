...
class MT5Provider(MarketDataProvider):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.name = "mt5"
        self.terminal_path = self.config.get("terminal_path", "C:/Program Files/Pepperstone MetaTrader 5/terminal64.exe")
    ...
    def get_historical_bars(
        ...
        for attempt in range(self.retries):
            try:
                ...
                print(f"[MT5] Historical error (attempt {attempt+1}/{self.retries}) for {symbol}: {e}")
                if attempt == self.retries - 1:
                    return []
                import time
                time.sleep(1)
