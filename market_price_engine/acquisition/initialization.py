...
class HistoricalInitializer:
    ...
    FX_SYMBOLS = [
        ...
        'AUDJPY', 'CADJPY', 'CHFJPY', 'NZDJPY', 'EURCHF',
        'GBPCHF', 'AUDNZD', 'AUDCAD', 'NZDCAD', 'GBPNZD',
        'EURNZD', 'EURCAD', 'GBPAUD', 'GBPCAD', 'CADCHF'
    ]
    
    # Indices
    INDICES = [
        'US500', 'US100', 'US30', 'GER40', 'UK100',
        'FRA40', 'JP225', 'HK50', 'AU200', 'EU50'
    ]
    
    # Commodities
    COMMODITIES = [
        'XAUUSD', 'XAGUSD', 'WTI', 'Brent', 'Natural Gas',
    ...
    def initialize_90_days(
        ...
        for symbol in target_symbols:
            ...
            for timeframe in target_timeframes:
                attempt_count += 1
                print(f"  [{attempt_count}/{total_attempts}] Loading {symbol} ({timeframe})...")
            ...
            print(f"  Summary {symbol}: {symbol_success}/{len(target_timeframes)} timeframes loaded")
        
        results["end_time"] = datetime.now().isoformat()
        results["success_rate"] = (results["successful"] / total_attempts) * 100 if total_attempts > 0 else 0
    ...
    def _initialize_symbol_timeframe(self, symbol: str, timeframe: str) -> Dict[str, Any]:
    ...
    def _load_from_provider(self, provider, symbol: str, timeframe: str, days: int) -> List[OHLCVData]:
