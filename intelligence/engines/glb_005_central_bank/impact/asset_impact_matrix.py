...
from intelligence.schemas.asset_impact import (
    AssetImpact, AssetImpactMatrix, AssetType, Direction, ImpactStatus, ImpactDriver
)
...
class AssetImpactMatrixGenerator:
    ...
    SUPPORTED_ASSETS = [
        ...
        "XAUUSD", "XAGUSD",
        "US500", "US100", "US30", "GER40", "UK100", "JP225"
    ]
    ...
    @classmethod
    def generate(cls, banks: List[Any], divergence: Dict, rate_environment: Dict, confidence: float) -> AssetImpactMatrix:
        ...
        for asset in cls.SUPPORTED_ASSETS:
            impact = cls._calculate_asset_impact(asset, policy_profiles, divergence, rate_environment, confidence)
    ...
    @classmethod
    def _calculate_asset_impact(cls, asset: str, profiles: Dict, divergence: Dict, 
                               rate_env: Dict, confidence: float) -> AssetImpact:
        ...
        if len(currencies) == 2:
            ...
            impact = cls._calculate_fx_impact(asset, base, quote, profiles, divergence, confidence)
        else:
            # Non-FX asset
            impact = cls._calculate_other_impact(asset, currencies, profiles, rate_env, confidence)
        
        return impact
    
    @classmethod
    def _calculate_fx_impact(cls, asset: str, base: str, quote: str, 
                            profiles: Dict, divergence: Dict, confidence: float) -> AssetImpact:
        ...
        if abs(policy_diff) > 10:
            drivers.append(f"{base_bank} policy score: {base_score:.1f} vs {quote_bank}: {quote_score:.1f}")
        ...
        return AssetImpact(
            ...
            status=ImpactStatus.ANALYZED if abs(score) > 5 else ImpactStatus.NOT_COVERED,
            drivers=[ImpactDriver(name=d, direction=direction, strength=0.7) for d in drivers[:3]],
    ...
    @classmethod
    def _calculate_other_impact(cls, asset: str, currencies: List[str], 
                               profiles: Dict, rate_env: Dict, confidence: float) -> AssetImpact:
        ...
        return AssetImpact(
            ...
            status=ImpactStatus.ANALYZED if abs(score) > 5 else ImpactStatus.NOT_COVERED,
            drivers=[ImpactDriver(name=d, direction=direction, strength=0.6) for d in drivers[:2]],
            relevance=0.5,
            engine_id="GLB-005",
            engine_name="Central Bank Intelligence Engine"
        )
