...
class AssetImpactMatrixGenerator:
    ...
    @classmethod
    def generate(cls, flow_analysis: Dict, liquidity_analysis: Dict, confidence: float) -> AssetImpactMatrix:
        ...
        for asset in cls.SUPPORTED_ASSETS:
            impact = cls._calculate_asset_impact(
                asset, flow_strength, flow_direction, 
                liquidity_score, liquidity_state, funding_stress,
                dominant_flow, confidence
            )
    ...
    @classmethod
    def _calculate_asset_impact(cls, asset: str, flow_strength: float, flow_direction: str,
                               liquidity_score: float, liquidity_state: str, funding_stress: float,
                               dominant_flow: str, confidence: float) -> AssetImpact:
        ...
        if "risk_off" in exposure:
            ...
            if flow_direction == "OUTFLOW" or "RISK_OFF" in dominant_flow or "SAFE_HAVEN" in dominant_flow:
        ...
        if liquidity_score < 40:
            ...
            elif asset_type == AssetType.FX and asset not in ["USDCHF", "USDJPY", "XAUUSD"]:
        ...
        impact_drivers = []
        for d in drivers[:3]:
            impact_drivers.append(ImpactDriver(
                name=d,
                direction=direction,
