...
from ..constants import CapitalFlowType, FlowDirection, FlowMomentum
...
class FlowAnalyzer:
    ...
    def analyze_flows(self, flows: List[CapitalFlowInput]) -> Dict[str, Any]:
        ...
        total_inflow = sum(f.amount_normalized for f in flows if f.direction == FlowDirection.INFLOW)
        total_outflow = sum(f.amount_normalized for f in flows if f.direction == FlowDirection.OUTFLOW)
        ...
        for flow in flows:
            flow_types[flow.flow_type.value] += flow.amount_normalized * flow.confidence / 100
        
        dominant_flow = max(flow_types.items(), key=lambda x: x[1])[0] if flow_types else "UNKNOWN"
        ...
        avg_amount = sum(f.amount_normalized for f in flows) / total_flows if total_flows > 0 else 0
        avg_confidence = sum(f.confidence for f in flows) / total_flows if total_flows > 0 else 0
    ...
    def _calculate_momentum(self, flows: List[CapitalFlowInput]) -> FlowMomentum:
        ...
        previous_avg = sum(f.amount_normalized for f in flows[:-1]) / (len(flows) - 1) if len(flows) > 1 else current_avg
        ...
        elif abs(current_avg - previous_avg) / previous_avg > 0.5 if previous_avg > 0 else False:
    ...
    def _group_by_asset_class(self, flows: List[CapitalFlowInput]) -> Dict[str, float]:
        ...
        asset_classes = {
            "XAUUSD": "COMMODITY", "XAGUSD": "COMMODITY", "WTI": "COMMODITY", "BRENT": "COMMODITY",
            "US500": "EQUITY", "US100": "EQUITY", "US30": "EQUITY",
            "GER40": "EQUITY", "UK100": "EQUITY", "JP225": "EQUITY",
            "EURUSD": "FX", "GBPUSD": "FX", "USDJPY": "FX",
            "AUDUSD": "FX", "NZDUSD": "FX", "USDCAD": "FX", "USDCHF": "FX",
        }
        ...
        for asset_class in class_flows:
            if class_counts[asset_class] > 0:
                class_flows[asset_class] = min(100, class_flows[asset_class] / class_counts[asset_class])
