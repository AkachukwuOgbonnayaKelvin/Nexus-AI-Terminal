...
def detect_pullback(mtf_structure: Dict) -> Optional[Dict]:
    ...
    lower_bullish = all(timeframes.get(tf, {}).get('bias') == primary_bias for tf in execution_tfs if tf in timeframes)
...
def calculate_pullback_zone(df: pd.DataFrame, atr: float, current_price: float, bias: str) -> Dict:
    depth = 1.2 * atr
    if bias == 'bullish':
...
def estimate_time_to_zone(current_price: float, zone_low: float, zone_high: float,
                          atr: float, timeframe: str = 'H1') -> Dict:
    """
    ...
    if current_price > zone_high:
        ...
        return {'hours_min': 0, 'hours_max': 0, 'direction': 'inside', 'distance_atr': 0.0}
    ...
    if hourly_move <= 0:
        return {'hours_min': 0, 'hours_max': 0, 'direction': direction, 'distance_atr': distance / atr}
